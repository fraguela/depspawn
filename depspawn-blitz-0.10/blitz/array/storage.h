...
public:
	...
	ordering_(x.ordering_), base_(x.base_),
        originalDims_(x.originalDims_)
    {
      for (int i=0; i < originalDims_; ++i) {
        extent_[i] = x.extent_[i];
        originalDim_[i] = x.originalDim_[i];
      }
    }
    ...
    GeneralArrayStorage(TinyVector<int,N_rank> ordering,
      ...
      : paddingPolicy_(pp), ascendingFlag_(ascendingFlag),
        ordering_(ordering), originalDims_(N_rank), originalDim_(tensor::i)
    {
        base_ = 0;
    }
	...
	paddingPolicy_ = rhs.paddingPolicy_;
        ...
        originalDims_ = rhs.originalDims_;
        for (int i=0; i < originalDims_; ++i) {
          extent_[i] = rhs.extent_[i];
          originalDim_[i] = rhs.originalDim_[i];
        }
        return *this;
    }
    ...
    int originalDims() const { return originalDims_; }
  
    const TinyVector<int, 2>* extent() const { return extent_; }
  
    const TinyVector<int, 11>& originalDim() const { return originalDim_; }
  
    // Only invoked from setupStorage() where originalDim_ == tensor::i, so no indirection required
    void setExtent(int i, int length) {
      extent_[i] = TinyVector<int, 2>(base_[i], base_[i] + length - 1);
    }

    // i is the number of dimension seen by the user
    void displaceExtent(int i, const Range& r) {
      i = originalDim_[i];
      const int init_range = extent_[i][0];
      extent_[i] = TinyVector<int, 2>(r.first() + init_range, r.last() + init_range);
    }

    template<int ANYDIM>
    void copyDepSpawnInfo(const GeneralArrayStorage<ANYDIM>& other) {
      originalDims_ = other.originalDims();
      for (int i=0; i < originalDims_; ++i) {
        extent_[i] = other.extent()[i];
        originalDim_[i] = other.originalDim()[i];
      }
    }
  
    void remap(int new_user_dim, int old_user_dim) {
      originalDim_[new_user_dim] = originalDim_[old_user_dim];
    }
...
protected:
    ...
    int originalDims_; // DepSpawn
    TinyVector<int, 2> extent_[11]; //DepSpawn
    TinyVector<int, 11> originalDim_; //DepSpawn
};
