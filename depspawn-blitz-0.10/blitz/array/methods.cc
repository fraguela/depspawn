...
void Array<P_numtype, N_rank>::dumpStructureInformation(ostream& os) const
{
  ...
  // DepSpawn info dump
  const int od = storage_.originalDims();
  const TinyVector<int, 2> * extent = storage_.extent();
  const TinyVector<int, 11>& originaldim = storage_.originalDim();

  os << "Data=" << data_ << endl;
  os << "block=" << this->block() << endl;
  os << "From orix " << od << "D array: ";
  for (int i = 0; i < od; i++) {
    os << (i ? 'x' : ' ') << " [" << extent[i][0] << ", " << extent[i][1] << "] ";
  }
  os << endl << "With indirections : ";
  for (int i = 0; i < N_rank; i++) {
    os << " [" << i << " -> " << originaldim[i] << "] ";
  }
  os << endl;
...
_bz_inline2 void Array<P_numtype, N_rank>::setupStorage(int lastRankInitialized)
{
    ...
    for (int i=0; i <= lastRankInitialized; ++i)
      storage_.setExtent(i, length_[i]);
